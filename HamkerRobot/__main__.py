...
MERSAL_IMG = ( "https://telegra.ph//file/a47f16c936dbbd4165399.jpg",
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            image = random.choice(MERSAL_IMG)
            ...
            query.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton(text="⬅ ʙᴀᴄᴋ", callback_data="help_back"),
                      InlineKeyboardButton(text="⬅ ʜᴏᴍᴇ", callback_data="mersal_back")]]
                ),
            )
...
def mersal_about_callback(update, context):
    query = update.callback_query
    if query.data == "mersal_back":
        mersal_img = random.choice(MERSAL_IMG)
        query.message.edit_text(
                PM_START_TEXT.format(mersal_img),
                reply_markup=InlineKeyboardMarkup(buttons),
                parse_mode=ParseMode.MARKDOWN,
                timeout=60,
        )
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}","[SUPER SAIYAN MERSAL IS BACK](http://telegra.ph/file/dad52dfba82ba6ac43785.jpg)", parse_mode=ParseMode.MARKDOWN) 
        except Unauthorized:
            LOGGER.warning(
    ...
    if WEBHOOK:
        ...
        LOGGER.info("Mersal is now alive and functioning")
